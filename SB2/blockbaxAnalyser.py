...
class blockbaxAnalyser():
    ...
    def loadData(self, ext=False,cut=True,ref=False):
        for i in range(len(self.sb_numbers)):
            ...
            batt_path = os.path.join(os.path.abspath(__file__).removesuffix('blockbaxAnalyser.py'), 
                str(self.sb_numbers[i]), str(self.sb_numbers[i]) + '_blockbax_batt.csv')
            if self.ref:
                temp_path = os.path.join(os.path.abspath(__file__).removesuffix('blockbaxAnalyser.py'), 
            ...
            batt_df = pd.read_csv(batt_path, delimiter=',"', header=1, engine='python', names=names).replace('"','', regex=True)

            temp_df['Value'] = pd.to_numeric(temp_df['Value'], downcast="float")
            y_df['Value'] = pd.to_numeric(y_df['Value'], downcast="float")
            batt_df["Value"] = pd.to_numeric(batt_df["Value"], downcast="float")

            # Delete before and after
            data = [temp_df["Date"], temp_df["Value"], y_df["Value"], batt_df["Value"]]
            headers = ['Datetime', 'Temperature', 'Y value', 'Battery']
            ...
            if ext == True:
                ...
                ext_data = [data["Datetime"], data["Temperature"], data["Y value"], data["Battery"]]
                ext_headers = ['Datetime', 'Temperature', 'Y value', 'Battery']
    ...
    def loadRefData(self,filename):
        """
        Loads the reference data from the specified file into a dataframe from the CSV-formatted .txt file. 
        """
        names = ["Datetime", "Angle Y", "Angle X", "Temperature"]
    ...
    def determineOffsets(self,zero_date_start, zero_date_end):
        """
        Determines the mounting offset values by taking the mean of the data captured at a temperature of 20 degrees Celcius and an inclination of 0 degrees setting 
        """
        ...
        zero_df = zero_df.loc[(zero_df.index < delete_after)]
        offset = zero_df["Angle Y"].mean()
        self.refdf["Angle Y"] -= offset
        self.offsets.append(offset)

    def cleanData(self):
        """
        Gets rid of the data outside the specified temperature bounds (indicated by timestamps) so as to keep only the data associated with a certain temperature setting.
        """

        # keep between
        m20_begin = pd.Timestamp("2023-01-19 19:00:00")
        m20_end = pd.Timestamp("2023-01-20 22:00:00")
        # and
        m10_begin = pd.Timestamp("2023-01-20 23:00:00")
        m10_end = pd.Timestamp("2023-01-22 02:00:00")
        # and
        p0_begin = pd.Timestamp("2023-01-22 03:00:00")
        p0_end = pd.Timestamp("2023-01-23 06:00:00")
        # and
        p10_begin = pd.Timestamp("2023-01-23 07:00:00")
        p10_end = pd.Timestamp("2023-01-24 10:00:00")
        # and
        p20_begin = pd.Timestamp("2023-01-24 11:00:00")
        p20_end = pd.Timestamp("2023-01-25 14:00:00")
        # and
        p30_begin = pd.Timestamp("2023-01-25 15:00:00")
        p30_end = pd.Timestamp("2023-01-26 18:00:00")
        # and
        p40_begin = pd.Timestamp("2023-01-26 19:00:00")
        p40_end = pd.Timestamp("2023-01-27 22:00:00")
        # and
        p50_begin = pd.Timestamp("2023-01-27 23:00:00")
        p50_end = pd.Timestamp("2023-01-29 02:00:00")
    ...
    def matchRefData(self):
        """
        Find for each datatime index of the Smartbrick data, the closest (in time) reference datapoint.
        """
        for i in range(len(self.sb_numbers)):
        ...
        # Finding closest matching datetime in self.refdf: For each row, calculate time difference (absolute). Select smallest time difference
            ...
            for index,row in self.dfs[i].iterrows():
                ...
                ref_column.append(self.refdf.loc[min_index,"Angle Y"])
                ref_temp_column.append(self.refdf.loc[min_index, "Temperature"])
    ...
    def trendlineYcompTemp(self,i, y_index, y_value):
        """
        Calculates the slope, intercept and r-squared for a trendline of a set with angles.
        """
        if self.slope == None:
            self.slope = []
        if self.intercept ==None:
        ...
        temparray = np.array(self.dfs[i].groupby("Y setting").get_group(y_value)['Temperature'].tolist())
        yarray = np.array(self.dfs[i].groupby("Y setting").get_group(y_value)['Y Comp'].tolist())
        ...
        print("----------------------------------------------------------------------------------------------------")
        print("Trendline on compensated data of SmartBrick ", self.sb_numbers[i], "at angle ", y_value)
        print("slope: %f, intercept: %f" % (self.slope[y_index], self.intercept[y_index]))
        print("R-squared: %f" % r_value**2)

    def trendlineYrawTemp(self,i, y_index, y_value):
        """
        Calculates the slope, intercept and r-squared for a trendline of a set with angles.
        """
        if self.slope == None:
        ...
        temparray = np.array(self.dfs[i].groupby("Y setting").get_group(y_value)['Temperature'].tolist())
        yarray = np.array(self.dfs[i].groupby("Y setting").get_group(y_value)['Y value'].tolist())
        ...
        print("----------------------------------------------------------------------------------------------------")
        print("Trendline on data of SmartBrick ", self.sb_numbers[i], "at angle ", y_value)
        print("slope: %f, intercept: %f" % (self.slope[y_index], self.intercept[y_index]))
        print("R-squared: %f" % r_value**2)
    ...
    def dataPrep_nooffset(self, ref_filename):
        '''
        Data preparation but without subtracting offset acquired at T = 20 deg and inclination = 0 deg 
        '''
        self.loadData()
        self.loadRefData(ref_filename)
        #self.determineOffsets(offset_date_start,offset_date_end)
    ...
    def groupData(self):
        """
        Groups the data in the inclination/temperature groups for easy selection by adding a column with the group value.
        """
        for i in range(len(self.sb_numbers)):
            self.dfs[i]["Y setting"] = pd.cut(self.dfs[i]["Reference"], bins = self.bins_y, labels=self.y_steps)
            self.dfs[i]["Temp setting"] = pd.cut(self.dfs[i]["Temperature"], bins = self.bins_temp, labels=self.temp_steps)

    def groupData2(self,threshold=1.0):
        """
        UNFINISHED
        Groups data using an algorithm based on difference with previous value.
        """
        ygrouplist = []
        tempgrouplist = []
        index = 0
        ...
        for i in range(len(self.sb_numbers)):
            for index, row in self.dfs[i].iterrows():
                if index == 0: # Start condition
                    pass
                    
                if (row['Y value']-prevy)>threshold:
                    pass
    ...
    def standardScale(self):
        """
        Groups the data and standard transform the data, appending it in a column named xx stdtf.
        """
        self.groupData()
    ...
    def splitData(self):
        """
        Split the total dataset into a test and train set.
        """
